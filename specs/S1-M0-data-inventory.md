...
1. Are there obvious metadata inconsistencies (sample rate, channels, missing files)?
1. Are transcripts plausibly usable (blank / mismatch signals)?
1. Rough estimate of long-silence / noise issues (coarse metrics only).
1. Initial conclusion: do we need major cleanup, and what are dominant failure modes?
...
1. `inventory_files.csv` (per-file metadata table)
1. `inventory_report.md` (human-readable 1–2 pages, markdown)
1. `inventory_samples.csv` (sampled transcript checks list with manual columns)
...
**Fallback**: If audio is unreadable or resampling fails, set all silence
metrics to null.
...
- If a duration bin has fewer files than allocated samples, distribute extras
  proportionally to other bins
- Sampling must be deterministic by `--seed` (use stratified sampling with
  fixed seed)
...
- Number of files, total duration (hours)
...
- Use streaming read when possible (or read minimal to compute duration if
  library supports)
